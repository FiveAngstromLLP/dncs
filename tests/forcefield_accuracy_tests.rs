...
mod forcefield_accuracy_tests {
    ...
    fn test_energy_calculation_consistency() {
        ...
        assert!((energy1 - energy2).abs() < TOLERANCE, 
                "Energy calculation not consistent: {} vs {}", energy1, energy2);
        
        // Energy should be finite
        assert!(energy1.is_finite(), "Energy calculation returned non-finite value: {}", energy1);
    }
    ...
    fn test_energy_scaling() {
        ...
        for seq in sequences {
            ...
            assert!(energy.is_finite(), "Non-finite energy for sequence {}: {}", seq, energy);
        }
        ...
        assert!(energies.iter().any(|&e| e.abs() > 1.0), 
                "All energies are suspiciously small: {:?}", energies);
    }
    ...
    fn test_force_components() {
        ...
        assert!(system.particles.len() >= expected_atoms, 
                "System should have at least {} atoms, got {}", 
                expected_atoms, system.particles.len());
    }
    ...
    fn test_forcefield_variants() {
        ...
        for ff_type in forcefields {
            ...
            assert!(energy.is_finite(), 
                    "Energy calculation failed for forcefield {:?}: {}", ff_type, energy);
            
            energies.push((ff_type, energy));
        }
        
        // All forcefields should produce finite energies
        assert_eq!(energies.len(), 4, "Should have energies for all 4 forcefields");
        
        // Energies should be different between forcefields (they use different parameters)
        let first_energy = energies[0].1;
        let all_same = energies.iter().all(|(_, e)| (e - first_energy).abs() < TOLERANCE);
        assert!(!all_same, "All forcefields produce identical energies, which is suspicious");
    }
    ...
    fn test_energy_properties() {
        ...
        assert!(energy < 10000.0, 
                "Energy suspiciously high: {} kcal/mol", energy);
        
        // Energy shouldn't be extremely negative either
        assert!(energy > -100000.0, 
                "Energy suspiciously low: {} kcal/mol", energy);
    }
    ...
    fn test_charged_residues() {
        ...
        assert!(energy.is_finite(), 
                "Energy calculation failed for charged sequence: {}", energy);
    ...
    fn test_system_initialization() {
        ...
        assert!(!system.particles.is_empty(), "System should still have particles after init");
        
        // Check that atoms have proper types assigned
        let atoms_with_types = system.particles.iter()
            .filter(|atom| atom.atomtype.is_some())
            .count();
        
        assert!(atoms_with_types > 0, 
                "No atoms have types assigned after initialization");
        
        // Check that we have proper bonding information
        assert!(!system.firstbonded.is_empty(), 
    ...
    fn test_pdb_energy_calculation() {
        ...
        if std::path::Path::new(pdb_file).exists() {
            ...
            assert!(energy.is_finite(), 
                    "Energy calculation failed for PDB file: {}", energy);
    ...
    fn test_distance_calculations() {
        ...
        assert!(energy.is_finite(), "Energy should be finite for distance test");
    }
    ...
    fn test_forcefield_parameters() {
        ...
        assert!(!ff.atom_types.types.is_empty(), 
                "Forcefield should have atom types");
        assert!(!ff.residues.residue.is_empty(), 
                "Forcefield should have residue definitions");
        assert!(!ff.harmonic_bond_force.bonds.is_empty(), 
                "Forcefield should have bond parameters");
        assert!(!ff.harmonic_angle_force.angles.is_empty(), 
                "Forcefield should have angle parameters");
        assert!(!ff.nonbonded_force.atoms.is_empty(), 
                "Forcefield should have nonbonded parameters");
        
        // Test parameter ranges
        for atom_type in &ff.atom_types.types {
            assert!(atom_type.mass > 0.0, 
                    "Atom mass should be positive: {}", atom_type.mass);
        }
        
        for bond in &ff.harmonic_bond_force.bonds {
            assert!(bond.k >= 0.0, 
                    "Bond force constant should be non-negative: {}", bond.k);
            assert!(bond.length > 0.0, 
                    "Bond length should be positive: {}", bond.length);
        }
        
        for nb_atom in &ff.nonbonded_force.atoms {
            assert!(nb_atom.sigma > 0.0, 
                    "Sigma parameter should be positive: {}", nb_atom.sigma);
            assert!(nb_atom.epsilon >= 0.0, 
                    "Epsilon parameter should be non-negative: {}", nb_atom.epsilon);
        }
    }
    ...
    fn test_energy_units() {
        ...
        assert!(energy > -1000.0 && energy < 1000.0, 
                "Energy {} kcal/mol is outside expected range for small peptide", energy);
    }
    ...
    fn test_performance() {
        ...
        assert!(duration.as_secs() < 10, 
                "Energy calculation took too long: {:?}", duration);
    ...
    fn test_error_handling() {
        ...
        assert!(system.particles.is_empty(), "Empty sequence should produce no particles");
        ...
        for seq in sequences {
            for ff_type in &forcefields {
                ...
                assert!(!system.particles.is_empty(), 
                        "System should have particles for sequence {}", seq);
                ...
                println!("Sequence: {}, FF: {:?}, Energy: {:.3} kcal/mol", 
                         seq, ff_type, energy);
            }
        }
    }
