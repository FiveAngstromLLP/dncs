...
fn main() {
    ...
    for i in 0..10.min(sys.particles.len()) {
        ...
        println!("Atom {}: {} charge={:.3} pos=({:.3}, {:.3}, {:.3})", 
                 atom.serial, atom.name, atom.charge, 
        ...
        for j in (i+1)..sys.particles.len() {
            ...
            if distance < 0.15 {
                ...
                let elec_energy = if distance > 0.001 {
                    ...
                    if atom_i.charge * atom_j.charge > 0.0 { f64::INFINITY } else { f64::NEG_INFINITY }
                };
    ...
    println!("Found {} atom pairs with distance < 0.15 nm:", short_distances.len());
    for (i, (idx1, idx2, dist, lj, elec)) in short_distances.iter().take(10).enumerate() {
        ...
        println!("{}. Atoms {}({}) - {}({}) dist={:.4}nm LJ={:.1} Elec={:.1} q1={:.3} q2={:.3}",
                 i+1, atom1.serial, atom1.name, atom2.serial, atom2.name, 
                 dist, lj, elec, atom1.charge, atom2.charge);
    }
    
    let total_lj: f64 = short_distances.iter().map(|(_, _, _, lj, _)| if lj.is_finite() { *lj } else { 1000.0 }).sum();
    let total_elec: f64 = short_distances.iter().map(|(_, _, _, _, elec)| if elec.is_finite() { *elec } else { if *elec > 0.0 { 1000.0 } else { -1000.0 } }).sum();
